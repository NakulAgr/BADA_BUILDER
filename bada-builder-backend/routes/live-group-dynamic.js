...
router.post('/admin/projects/bulk', authenticate, isAdmin, upload.fields([
    ...
    try {
        ...
        const {
            ...
            property_types, property_configs
        } = req.body;
        ...
        try {
            ...
            // --- Multi-property Fallback Logic ---
            let finalPropertyTypes = Array.isArray(property_types) ? property_types : (typeof property_types === 'string' ? JSON.parse(property_types || '[]') : []);
            let finalPropertyConfigs = typeof property_configs === 'string' ? JSON.parse(property_configs || '{}') : (property_configs || {});

            // Fallback: if only one type or empty, use existing column logic and null out new columns to save space/complexity
            if (finalPropertyTypes.length <= 1) {
                finalPropertyTypes = null;
                finalPropertyConfigs = null;
            }
            ...
            const projectResult = await client.query(
                ...
                [
                    ...
                    finalPropertyTypes, finalPropertyConfigs
                ]
            );
            ...
            for (const towerData of hierarchyData) {
                ...
                const towerResult = await client.query(
                    ...
                    [project.id, towerData.tower_name || towerData.name || 'Block', towerData.total_floors || 0, towerData.layout_columns || null, towerData.layout_rows || null]
                );
