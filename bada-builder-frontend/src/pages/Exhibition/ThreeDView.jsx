...
const FLOOR_HEIGHT = 3.2; // Taller for High-rise feel
const UNIT_WIDTH = 6;     // Wider units
const UNIT_DEPTH = 6;
const UNIT_GAP = 0.5;
...
const ResidentialColony = ({ position, propertyData, project, onUnitClick }) => {
    ...
    while (i < allUnits.length) {
        ...
        const typeNorm = (unit.unit_type || '').toLowerCase().trim();
        const numNorm = (unit.unit_number || '').toLowerCase().trim();

        const isTV = typeNorm.includes('twin') || numNorm.includes('twin');
        const isPlot = typeNorm.includes('plot') || typeNorm.includes('land') || numNorm.includes('plot') || numNorm.includes('land');
        const isApartment = typeNorm.includes('apartment') || typeNorm.includes('flat') || typeNorm.includes('tower') || typeNorm.includes('apart') ||
            numNorm.includes('flat') || numNorm.includes('tower') || numNorm.includes('apart');
        const isCommercial = typeNorm.includes('commercial') || typeNorm.includes('shop') || typeNorm.includes('office') ||
            numNorm.includes('commercial') || numNorm.includes('shop') || numNorm.includes('office');
        ...
        if (isCommercial) {
            buildings.push({ type: 'Commercial', units: [unit] });
            i++;
            continue;
        }

        if (isApartment) {
            // Group ALL consecutive apartment units into one Tower building for this colony grid cell
            const apUnits = [];
            let j = i;
            while (j < allUnits.length) {
                const u = allUnits[j];
                const tn = (u.unit_type || '').toLowerCase();
                const un = (u.unit_number || '').toLowerCase();
                const isAp = tn.includes('apartment') || tn.includes('flat') || tn.includes('tower') ||
                    un.includes('flat') || un.includes('tower') || un.includes('apart');
                if (isAp) {
                    apUnits.push(u);
                    j++;
                } else {
            ...
            if (apUnits.length > 0) {
                buildings.push({ type: 'Apartment', units: apUnits });
                i = j;
                continue;
            }
        }
        ...
        if (isTV && i + 1 < allUnits.length) {
            ...
            const nextTypeNorm = (nextUnit.unit_type || '').toLowerCase();
    ...
    const towerType = (project?.towers?.[0]?.property_type || 'bungalow').toLowerCase();
    const config = project?.property_configs?.[towerType] || {};

    const SCALE = 0.2;
    const projectRW = (parseFloat(config.road_width) || parseFloat(project?.road_width) || 20) * SCALE;
    ...
    const columns = parseInt(config.layout_columns) || parseInt(project?.layout_columns) || parseInt(project?.towers?.[0]?.layout_columns) || 4;
    const rows = parseInt(config.layout_rows) || parseInt(project?.layout_rows) || parseInt(project?.towers?.[0]?.layout_rows) || Math.ceil(buildings.length / columns);
    ...
    return (
        <group position={position}>
                ...
                if (b.type === 'TwinVilla') {
                    ...
                    return (
                        <CommercialUnit
                            ...
                            visualNumber={idx + 1}
                            floorCount={parseInt(project?.towers?.[0]?.total_floors) || 2} // Use config or taller default
                        />
                    );
                    ...
                    return (
                        <Tower
                            ...
                            position={[xPos, 0, zPos]}
                            tower={{
                                tower_name: b.units[0].unit_number.includes('Flat') ? "Apartment Block" : b.units[0].unit_number,
                                units: b.units,
                                layout_columns: parseInt(project?.property_configs?.apartment?.layout_columns) || parseInt(project?.layout_columns) || parseInt(project?.towers?.[0]?.layout_columns) || 4,
                                total_floors: parseInt(project?.property_configs?.apartment?.total_floors) || parseInt(project?.towers?.[0]?.total_floors) || 0
                            }}
                            onUnitClick={onUnitClick}
                            lowestFloor={0}
                        />
            ...
            <Text
                ...
                anchorX="center"
            >
                {project.towers?.[0]?.tower_name || propertyData?.title || ''}
            </Text>
        </group>
    );
};
...
const CommercialColony = ({ position, project, onUnitClick }) => {
    const tower = project.towers?.[0];
    const units = tower?.units || [];
    const config = project?.property_configs?.commercial || {};

    const columns = parseInt(config.layout_columns) || parseInt(tower?.layout_columns) || 4;
    const rows = Math.ceil(units.length / columns);

    const SCALE = 0.2;
    const colW = (parseFloat(tower?.plot_size_width) || 30) * SCALE;
    const colD = (parseFloat(tower?.plot_size_depth) || 40) * SCALE;
    const gap = 4;
    const roadW = (parseFloat(config.road_width) || parseFloat(project?.road_width) || 20) * SCALE;

    const SPACING_X = colW + gap;
    const SPACING_Z = colD + roadW;

    const colonyWidth = columns * SPACING_X;
    ...
    return (
        <group position={position}>
            ...
            <mesh position={[0, -0.2, 0]} receiveShadow>
                <boxGeometry args={[colonyWidth + 20, 0.2, colonyDepth + 20]} />
            ...
            {units.map((unit, idx) => {
                const col = idx % columns;
                const row = Math.floor(idx / columns);
                const x = -colonyWidth / 2 + col * SPACING_X + colW / 2;
                const z = -colonyDepth / 2 + row * SPACING_Z + colD / 2;
                ...
                return (
                    <CommercialUnit
                        ...
                        onUnitClick={onUnitClick}
                        visualNumber={idx + 1}
                        floorCount={parseInt(config.commercial_floor_count) || parseInt(tower?.total_floors) || 2}
                    />
                );
            })}
            ...
            {/* Label */}
            <Text
                position={[0, 15, -colonyDepth / 2 - 5]}
                fontSize={4}
                color="#1e293b"
                anchorX="center"
            >
                {tower?.tower_name || "Commercial Block"}
            </Text>
        </group>
    );
};
...
const PlotColony = ({ position, propertyData, project, onUnitClick, showPremium, selectedUnit }) => {
    ...
    const config = project?.property_configs?.plot || {};

    const rows = parseInt(config.layout_rows) || parseInt(project?.layout_rows) || project?.towers?.[0]?.layout_rows || 5;
    const cols = parseInt(config.layout_columns) || parseInt(project?.layout_columns) || project?.towers?.[0]?.layout_columns || 4;

    const roadWidth = parseFloat(config.road_width) || parseFloat(project?.road_width) || 20;
    const plotGap = parseFloat(config.plot_gap) || parseFloat(project?.plot_gap) || 2;
    const plotWidth = parseFloat(project?.plot_size_width) || project?.towers?.[0]?.plot_size_width || 30;
    const plotDepth = parseFloat(project?.plot_size_depth) || project?.towers?.[0]?.plot_size_depth || 40;
...
const Tower = ({ tower, position, onUnitClick, lowestFloor }) => {
    ...
    // Generate array of floors to render: [0, 1, ..., total_floors-1]
    const configFloors = parseInt(tower.total_floors) || 0;
    const maxFloorIndex = Math.max(configFloors - 1, ...unitsFloors.map(f => parseInt(f)), 0);
    const renderingFloors = Array.from({ length: maxFloorIndex + 1 }, (_, i) => i);

    return (
        <group position={position}>
            {/* Tower Name Label - Positioned above the highest possible floor */}
            <Text
                position={[0, getFloorY(maxFloorIndex) + UNIT_HEIGHT + 1.5, 0]}
                fontSize={2.5}
                color="#1e293b"
                anchorX="center"
            >
            ...
            {renderingFloors.map((floorNum) => {
                const units = unitsByFloor[floorNum] || [];
                const floorY = getFloorY(floorNum);
                const specialColor = floorNum === 0 ? '#64748b' : null;

                return (
                    <group key={`floor-${floorNum}`}>
                        {/* Always Render Floor Slab */}
                        <mesh position={[0, floorY - (UNIT_HEIGHT / 2), 0]} receiveShadow>
                            <boxGeometry args={[footprintWidth, 0.2, footprintDepth]} />
                            <meshStandardMaterial color="#cbd5e1" />
                        </mesh>

                        {/* Render Units if they exist for this floor */}
                        {units.map((unit, idx) => {
                    ...
                    viewMode === '3d' && (
                        ...
                        >
                            ...
                            {(() => {
                                ...
                                return project.towers.map((tower, idx) => {
                                    const towerType = (tower.property_type || '').toLowerCase().trim();
                                    const towerUnits = tower.units || [];
                                    const unitTypes = new Set(towerUnits.map(u => (u.unit_type || '').toLowerCase().trim()));

                                    // Robust type identification
                                    const isApartment = towerType.includes('apartment') ||
                                        towerType.includes('flat') ||
                                        towerType.includes('tower') ||
                                        tower.tower_name?.toLowerCase().includes('tower') ||
                                        tower.tower_name?.toLowerCase().includes('flat') ||
                                        [...unitTypes].some(t => t.includes('apartment') || t.includes('flat') || t.includes('tower'));

                                    const isCommercial = towerType.includes('commercial') ||
                                        towerType.includes('shop') ||
                                        tower.tower_name?.toLowerCase().includes('commercial') ||
                                        tower.tower_name?.toLowerCase().includes('shop') ||
                                        [...unitTypes].some(t => t.includes('commercial') || t.includes('shop') || t.includes('office'));

                                    const isPlot = towerType.includes('plot') ||
                                        towerType.includes('land') ||
                                        tower.tower_name?.toLowerCase().includes('plot') ||
                                        [...unitTypes].some(t => t.includes('plot') || t.includes('land'));

                                    // Spacing depends on type
                                    const spacing = (isApartment ? 50 : 120);
                                    const posX = (idx - (project.towers.length - 1) / 2) * spacing;
                                    ...
                                    // Default: Residential Colony (Bungalows, Twin Villas)
                                    return (
                                        <ResidentialColony
                                            key={tower.id || idx}
                                            position={[posX, 0, 0]}
