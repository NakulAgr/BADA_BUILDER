...
const LeadModal = ({ isOpen, onClose }) => {
  ...
  // Optimized auto-scroll function - reduced complexity
  const scrollToNextField = (currentField) => {
    // Only scroll for key fields to reduce performance impact
    const scrollFields = ['requirementType', 'bhkType'];
    if (!scrollFields.includes(currentField)) return;

    let nextField = null;
    if (currentField === 'requirementType') {
      const showBhkType = ['Flat', 'House', 'Villa'].includes(formData.requirementType);
      nextField = showBhkType ? 'bhkType' : null;
    }
  ...
  // Removed auto-scroll effects to prevent buffering - user can scroll manually
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const savePromise = addDoc(collection(db, 'leads'), leadData);
      ...
      setError(err.message === 'Request timeout' ? 'Request timed out. Please try again.' : 'Failed to submit. Please try again.');
    } finally {
      setLoading(false);
    }
  };
